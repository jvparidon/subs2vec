...
@timer
def train_fasttext(training_data, working_dir, cores, d=300, neg=5, epoch=5, t=.0001):
    binary = ['../local-bin/fasttext']
    method = ['skipgram']
    train = ['-input', training_data]
    new_fname = training_data.split('/')[-1].replace('.txt', '.neg{}.epoch{}.t{}.{}d'.format(neg, epoch, t, d))
    output = ['-output', os.path.join(working_dir, new_fname)]
    neg = ['-neg', str(neg)]
    ...
    model = '{}.bin'.format(new_fname)
    vecs = '{}.vec'.format(new_fname)
    return model, vecs


@timer
def build_phrases(training_data, working_dir, phrase_pass):
    base_fname = os.path.join(working_dir, training_data.split('/')[-1].replace('.txt', ''))
...
def generate(lang, working_dir, subs_dir, subs_prep, dedup, phrase_pass, cores):
    ...
    for lang in langs:
        ...
        if dedup:
            ...
            out_fname = os.path.join(working_dir, training_data.split('/')[-1].replace('.txt', '.dedup.txt'))
            results, t = deduplicate.dedup_file(training_data, out_fname)
            n_lines, n_duplicates = results
            training_data = out_fname
            print('read {} lines and removed {} duplicates in {} seconds'.format(n_lines, n_duplicates,
                                                                                 int(t['duration'])))
