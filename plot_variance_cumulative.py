...
def plot_variance(fnames):
    ...
    for i in range(len(fnames)):
        ...
        fname = fnames[i].replace('.explained_variance', '').split('/')[-1:][0]
        if 'binder' in fname:
            plt.plot(np.hstack([0.0, np.interp(np.arange(300), np.arange(65) * (300 / 65), np.cumsum(vecs))]), alpha=.5, label='{}'.format(fname))
            #plt.plot(np.hstack([0.0, np.interp(np.arange(300), np.arange(65) * (300 / 65), vecs)]), alpha=.5, label='{}'.format(fname))
        else:
            plt.plot(np.hstack([0.0, np.cumsum(vecs)]), alpha=.5, label='{}'.format(fname))
            #plt.plot(np.hstack([0.0, vecs]), alpha=.5, label='{}'.format(fname))
    plt.plot(np.hstack([0.0, np.cumsum(pca_normal())]), alpha=.5, label='Gaussian noise')
    #plt.plot(np.hstack([0.0, pca_normal()]), alpha=.5, label='Gaussian noise')
    ...
    plt.savefig('300d_pca_cumulative.png', dpi=100)
