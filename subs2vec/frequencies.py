...
if __name__ == '__main__':
    ...
    unigrams, bigrams, trigrams = count_ngrams_by_line(**vars(args))
    base_fname = '.'.join(args.filename.split('.')[:-1])
    unigrams.to_csv(f'{base_fname}.{args.kind}_unigram_freqs.tsv', index_label='unigram', sep='\t')
    bigrams.to_csv(f'{base_fname}.{args.kind}_bigram_freqs.tsv', index_label='bigram', sep='\t')
    trigrams.to_csv(f'{base_fname}.{args.kind}_trigram_freqs.tsv', index_label='trigram', sep='\t')
